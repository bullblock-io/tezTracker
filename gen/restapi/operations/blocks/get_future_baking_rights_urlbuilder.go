...
func (o *GetFutureBakingRightsURL) Build() (*url.URL, error) {
	...
	var _path = "/v2/data/{platform}/{network}/future_baking_rights"

	network := o.Network
	if network != "" {
		_path = strings.Replace(_path, "{network}", network, -1)
	} else {
		return nil, errors.New("network is required on GetFutureBakingRightsURL")
	}
	...
	if platform != "" {
		...
		return nil, errors.New("platform is required on GetFutureBakingRightsURL")
	}
...
func (o *GetFutureBakingRightsURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetFutureBakingRightsURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetFutureBakingRightsURL")
	}
